...
{
	auto UserPassword = static_cast<std::string*>(data);
...
int callbackInteger(void* data, int argc, char** argv, char** azColName)
{
	...
	if (argv[0] == nullptr) throw std::exception("Encountered an error while retrieving data");
...
{
	auto num = static_cast<double*>(data);
...
bool SqliteDataBase::open()
{
	...
	{
		sqlStatement =
			"CREATE TABLE USERS (USERNAME TEXT NOT NULL PRIMARY KEY UNIQUE, PASSWORD TEXT NOT NULL, EMAIL TEXT NOT NULL)";
		...
		sqlStatement =
			"CREATE TABLE QUESTIONS (ID INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, CATEGORY TEXT NOT NULL, QUESTIONS TEXT NOT NULL UNIQUE, A TEXT NOT NULL, B TEXT NOT NULL, C TEXT NOT NULL, D TEXT NOT NULL, ANSWER INTEGER NOT NULL)";
		res = sqlite3_exec(db, sqlStatement.c_str(), nullptr, nullptr, &errMessage);
		if (res != SQLITE_OK)
		{
			std::cerr << errMessage << std::endl;
		...
		sqlStatement =
			"CREATE TABLE STATISTICS(USERNAME INTEGER NOT NULL, NUM_OF_GAMES  INTEGER NOT NULL, AVG_TIME  FLOAT NOT NULL, CORRECT_ANSWERS  INTEGER NOT NULL, WRONG_ANSWERS INTEGER NOT NULL, FOREIGN KEY(USERNAME) REFERENCES USERS(USERNAME));";
		res = sqlite3_exec(db, sqlStatement.c_str(), nullptr, nullptr, &errMessage);
		if (res != SQLITE_OK)
		{
			std::cerr << errMessage << std::endl;
...
{
	auto sqlStatement = "DELETE FROM USERS;";
	char** errMessage = nullptr;
...
void SqliteDataBase::insertUserIntoDB(const std::string& username, const std::string& password,
                                      const std::string& email)
{
	std::string sqlStatement = "INSERT INTO USERS (USERNAME, PASSWORD, EMAIL) VALUES ('" + username + "', '" + password
		+ "', '" + email + "');";
	char* errMessage = nullptr;
...
{
	int total_answers = 0;
	std::string sqlStatement = "SELECT SUM(CORRECT_ANSWERS + WRONG_ANSWERS) FROM STATISTICS WHERE USERNAME = '" +
		username + "';";
	char* errMessage = nullptr;
...
std::vector<std::string> SqliteDataBase::getTopUserGrades() const
{
	...
	std::string sqlStatement =
		"SELECT USERNAME, ((CORRECT_ANSWERS * 1.0) / (CORRECT_ANSWERS + WRONG_ANSWERS)) / AVG_TIME * 10 AS grade "
		"FROM STATISTICS "
		"ORDER BY grade DESC "
		"LIMIT 5;";
	char* errMessage = nullptr;
	...
	{
		auto userGrades = static_cast<std::vector<std::string>*>(data);
		if (argc == 2)
		{
...
std::vector<std::string> SqliteDataBase::getQuestions(const int amount, std::string category) const
{
	...
	std::string sqlStatement = "SELECT QUESTIONS FROM QUESTIONS WHERE CATEGORY = '" + category +
		"' ORDER BY RANDOM() LIMIT " + std::to_string(amount) + ";";
	auto callback = [](void* data, int argc, char** argv, char** azColName) -> int
	{
		auto questionList = static_cast<std::vector<std::string>*>(data);
		if (argc > 0)
		{
...
std::vector<std::string> SqliteDataBase::getPossibleAnswers(const std::string& title) const
{
	...
	auto callback = [](void* data, int argc, char** argv, char** azColName) -> int
	{
		auto answerList = static_cast<std::vector<std::string>*>(data);
